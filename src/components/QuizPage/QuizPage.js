...
const QuizPage = () => {
  ...
  const checkAnswer = (quizOptions, correctAnswer) => {
    if (quizOptions === correctAnswer) {
      ...
      const alreadyAnswered = rightAnswers.find(rightAnswer => rightAnswer === quizOptions);
      if (!alreadyAnswered) {
      ...
      const alreadyAnswered = wrongAnswers.find(rightAnswer => rightAnswer === quizOptions);
      if (!alreadyAnswered) {
  ...
  return (
    ...
    <div className='quiz-page grid grid-cols-3 w-4/5 mx-auto relative'>
      ...
      <div className='counter-section sticky top-0 mt-9 ml-20 p-8 border border-blue-200 rounded'>
          <p className='text-green-700 text-xl font-semibold'>Right Answer : {rightAnswers.length}</p>
          <p className='text-red-600 text-xl font-semibold'>Wrong Answer : {wrongAnswers.length}</p>
      </div>
   </div>
